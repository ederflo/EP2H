...
public class Arena {
    ...
    public Arena(BattleSimulation sim, DinosaurCollection fighters) {
        this.sim = sim;
        this.roster = buildRoster(fighters);
    }
    ...
    private Dinosaur[] buildRoster(DinosaurCollection fighters) {
        Dinosaur[] result = new Dinosaur[fighters.size()];
        DinosaurIterator iter = fighters.iterator();
        int idx = 0;
        while (iter.hasNext())
            result[idx++] = iter.next();

        this.scoreSheet = new int[fighters.size()];
        return result;
    }
    ...
    public void runTournament() {
        ...
        for (int i = 0; i < roster.length; i++) {
            for (int j = i + 1; j < roster.length; j++) {
                ...
                if (winner != null && winner.equals(roster[i]))
                    scoreSheet[i]++;
                else if (winner != null && winner.equals(roster[j]))
                    scoreSheet[j]++;
            }
        }
    }
