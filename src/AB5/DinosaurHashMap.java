...
public class DinosaurHashMap implements HashMap {
    ...
    public DinosaurHashMap(int capacity, int maxBucketSize) {
        buckets = new BucketList[capacity > 0 ? capacity : DEFAULT_NUM_OF_BUCKETS];
        for (int i = 0; i < buckets.length; i++)
            buckets[i] = new DinosaurBucketList();
        this.maxBucketSize = maxBucketSize > 0 ? maxBucketSize : DEFAULT_MAX_BUCKET_SIZE;
    }
    ...
    private BucketList[] reorganizeBuckets(int newCapacity) {
        ...
        while (iterator.hasNext()) {
            ...
            result[getBucketIdx(dino)].store(dino);
        }
        return result;
    }
    ...
    public Dinosaur put(Dinosaur dinosaur) {
        int bucketIdx = getBucketIdx(dinosaur);
        Dinosaur result = buckets[bucketIdx].store(dinosaur);
        if (buckets[bucketIdx].size() > maxBucketSize)
    ...
    public Dinosaur get(DinosaurDNA dna) {
        return buckets[getBucketIdx(dna)].find(dna);
    }
    ...
    public Dinosaur remove(DinosaurDNA dna) {
        return buckets[getBucketIdx(dna)].remove(dna);
    }
    ...
    @Override
    public void clear() {
        buckets = new BucketList[buckets.length];
        for (int i = 0; i < buckets.length; i++)
            buckets[i] = new DinosaurBucketList();
    }
    ...
    public int size() {
        int size = 0;
        for (BucketList bucketList : buckets)
    ...
    private int getBucketIdx(Dinosaur dino) {
        return dino.hashCode() % maxBucketSize;
    }

    private int getBucketIdx(DinosaurDNA dna) {
        return dna.hashCode() % maxBucketSize;
    }
}
